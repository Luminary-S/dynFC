...
class URNode(FrameNode):
    ...
    def define_node_subscriber(self):
        ...
        force_sub = rospy.Subscriber("/robotiq_ft_wrench", WrenchStamped, self.callback_ft_sensor, queue_size=1)
...
### call back
    ...
    def callback_ft_sensor(self, msg):
        ...
        # print(self.force)
        # self.force = list( np.dot( UT.tr2jac( UT.TfromThetap([0,0,1/4*math.pi],[0,0,0]), 0 ) , np.array(force).T ).T )
    ...
    def buildMove(self, moveType="stop", pose = [83.84, -153.38, 23.84, 115.12, -102.87, 357.59 ]):
        ...
        if moveType[0] == "f":
            ...
            acceleration = 0.1  # Joint acceleration in rad/s^2
            speed = 0.07  # Joint speed in rad/s
